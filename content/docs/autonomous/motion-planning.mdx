...
**Motion planning** is the process of deciding **what path your robot should take** to reach its destination. It's the difference between blindly driving forward and intelligently navigating around obstacles, optimizing speed, and coordinating with game strategy.
...
public class WaypointAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        Pose2D[] waypoints = {
            ...
            new Pose2D(24, 12, 45),   // Avoid obstacle
...
class WaypointAuto : LinearOpMode() {
    ...
    override fun runOpMode() {
        ...
        val waypoints = listOf(
            ...
            Pose2D(24.0, 12.0, 45.0),   // Avoid obstacle
...
Advanced motion planning includes **avoiding obstacles** on the field.

### Simple Avoidance (Predefined Waypoints)

Plan paths around known obstacles (field elements):

```java
// Bad: drives through OBELISK
goToPoint(48, 48);

// Good: waypoints avoid OBELISK
goToPoint(24, 20); // Go around
goToPoint(48, 48); // Then to target
```

### Dynamic Avoidance (Sensor-Based)

Use distance sensors or vision to detect unexpected obstacles and reroute:

<Tabs items={['Java', 'Kotlin']}>
<Tab value="Java">
```java
public void driveWithAvoidance(double targetX, double targetY) {
    while (opModeIsActive()) {
        odometry.update();
        ...
        if (distance < 15) {
            // Obstacle detected! Strafe left
            drivetrain.drive(0, -0.3, 0);
        } else {
            // Clear path, navigate normally
            navigateToPoint(targetX, targetY);
        }
    }
}
```
...
<Tab value="Kotlin">
```kotlin
fun driveWithAvoidance(targetX: Double, targetY: Double) {
    while (opModeIsActive()) {
        odometry.update()
