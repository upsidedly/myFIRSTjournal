...
```math
\text{Predicted Position} = \text{Previous Position} + (\text{Velocity} \times \Delta t)
```
...
```math
\text{New Estimate} = \text{Prediction} + K (\text{Measurement} - \text{Prediction})
```

The **Kalman Gain** ($K$) automatically balances:
- How much to trust the prediction (process noise $Q$)
- How much to trust the sensor (measurement noise $R$)

Where:
- $\hat{x}_k$ = State estimate at time $k$
- $P_k$ = Error covariance (uncertainty)
