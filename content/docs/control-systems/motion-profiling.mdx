...
For a move distance `d` with max velocity `v_max` and max acceleration `a_max`:
...
**If distance allows full trapezoid:**
- Acceleration phase: `0` to `t_accel`
- Cruise phase: `t_accel` to `t_total - t_accel`
- Deceleration phase: `t_total - t_accel` to `t_total`

**If distance too short (triangular profile):**
- No cruise phase
